...
}:
{
  ...
  # No swap partition, so a memory spike in Stalwart would hit the OOM
  # killer straight away; compressed RAM instead of a disk partition - a
  # VPS root disk is small and the write amplification isn't worth it.
  #
  # 25% not redtruck's 50%: this box has ~1G and the zram device costs real
  # RAM as it fills - 256M of swap for roughly 100M resident at zstd.
  zramSwap = {
    enable = true;
    memoryPercent = 25;
  };
