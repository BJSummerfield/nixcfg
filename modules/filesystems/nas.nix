...
in
{
  ...
  config = mkIf hasEnabledShares {
      ...
      let
        ...
        groups = lib.concatLists (
          mapAttrsToList (
            shareName: level:
            let
              share = cfg.shares.${shareName} or null;
            in
            if share == null then
              [ ]
            else if level == "rw" && share.rwGid != null then
              [ "${shareName}-rw" ]
            else if level == "ro" && share.roGid != null then
              [ "${shareName}-ro" ]
            else
              [ ]
          ) access
          ...
          mapAttrsToList (
            ...
            in
            {
              assertion =
                ...
                || ((level == "rw" -> share.rwGid != null) && (level == "ro" -> share.roGid != null));
    ...
    home-manager.sharedModules = [
      ({ config, ... }: {
        home.file."nas".source = config.lib.file.mkOutOfStoreSymlink "/mnt/secure/nas";
      })
    ];
  };
}
